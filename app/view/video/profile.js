...
export default class VideoProfile extends Component {
    ...
    async getVideoDetail() {
      ...
      try {
        let response = await API.getVideoDetail(this.state.id)
        let responseData = await response.json()
        let video = responseData.Response[0]
        this.setState({
                                ...
                                <View style={{ flex:1, flexDirection:'row' }}>
                                    <View style={{ width:Constant.layout.magazineWidth, height:'100%', padding:5 }} activeOpacity={ 0.8 }>
                                        <View style={{ flex:1, justifyContent:'center' }}>
                                            <Image
                                              style={{ width:Constant.layout.magazineWidth - 10, height:Math.round((Constant.layout.magazineWidth - 10) * 1.5), backgroundColor:Constant.color.placeholder }}
                                        ...
                                        <View style={{ width:'100%' }}>
                                            <View style={{ flex:1, flexDirection:'row' }}>
                                                <View style={{ flex:1, justifyContent:'flex-end' }}>
                                                    <Text style={{ color:'#ddbb1f', fontSize:17, fontFamily:Constant.font.roman }}>{ video.title }</Text>
                                                </View>
                                            ...
                                            <View style={{ width:'100%' }}>
                                                <View style={{ flex:1, flexDirection:'row' }}>
                                                    { array }
                                                    <Text style={{ color:'#ddbb1f', fontSize:15, marginHorizontal:10 }}>{ video.rates + '.0' }</Text>
                                                </View>
                                            </View>
                            ...
                            <View style={{ width:'100%', marginVertical:30, marginHorizontal:(Constant.layout.screenWidth - 20 - 244) / 3 }}>
                                <View style={{ flex:1, flexDirection:'row' }}>
                                    <TouchableOpacity
                                      ...
                                      onPress={ () => {this.props.navigation.navigate('VideoViewer', {
                                          uri: video.uri,
                                          title: video.title
                                        })}
                                      }
                                    >
                                        <Image style={{ width:'100%', height:'100%', resizeMode:'stretch' }} source={ imgButton }>
                                            <View style={ [styles.wrapper, {paddingTop:Platform.OS == 'ios' ? 3: 0}] }>
                                                <Text style={{ color:'white', fontFamily:Constant.font.book, fontSize:10, backgroundColor:'transparent' }}>Play Video</Text>
                                            </View>
                                        </Image>
                                    </TouchableOpacity>
                                    <TouchableOpacity
