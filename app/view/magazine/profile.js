...
export default class MagazineProfile extends Component {
    constructor(props) {
        super(props)
        this.state = {
            rate: 0,
    ...
    async getMagazineDetail() {
        ...
        try {
            let response = await API.getMagazineDetail(this.state.id)
            let responseData = await response.json()
            let magazine = responseData.Response[0]
            this.setState({
              rate: parseInt(magazine.rates),
              magazine: magazine,
              comments: magazine.user_rates == null ? [] : magazine.user_rates,
              isLoading: false
            })
        }
        catch(error) {
                        ...
                        <ScrollView style={{ flex:1, paddingHorizontal:10 }}>
                            <View style={{ width:'100%' }}>
                                <View style={{ flex:1, flexDirection:'row' }}>
                                    <View style={{ width:Constant.layout.magazineWidth, height:'100%', padding:5 }} activeOpacity={ 0.8 }>
                                        <View style={{ flex:1, justifyContent:'center' }}>
                                            <TouchableOpacity
                                              style={{ width:Constant.layout.magazineWidth - 10, height:Math.round((Constant.layout.magazineWidth - 10) * 1.5) }}
                                              activeOpacity={ 0.8 }
                                              onPress={ () => {this.props.navigation.navigate('MagazineViewer')} } >
                                                <Image
                                        ...
                                        <View style={{ width:'100%' }}>
                                            <View style={{ flex:1, flexDirection:'row' }}>
                                                <View style={{ flex:1, justifyContent:'flex-end' }}>
                                                    <Text style={{ color:'#ddbb1f', fontSize:17, fontFamily:Constant.font.roman }}>{ magazine.title }</Text>
                                                </View>
                                            ...
                                            <View style={{ width:'100%' }}>
                                                <View style={{ flex:1, flexDirection:'row' }}>
                                                    { array }
                                                    <Text style={{ color:'#ddbb1f', fontSize:15, marginHorizontal:10 }}>{ magazine.rates + '.0' }</Text>
                                                </View>
                                            </View>
                                        </View>
                                    </View>
                            ...
                            <Text style={{ color:'white', fontSize:10, fontFamily:Constant.font.roman, marginVertical:20 }}>
                            Lorem ipsum dolor sit amet, consectetur adipiscing elit, sed do eiusmod tempor incididunt ut labore et dolore magna aliqua. Ut enim ad minim veniam, quis nostrud exercitation ullamco laboris nisi ut aliquip ex ea commodo consequat. Duis aute irure dolor in reprehenderit in voluptate velit esse cillum dolore eu fugiat nulla pariatur. Excepteur sint occaecat cupidatat non proident, sunt in culpa qui officia deserunt mollit anim id est laborum.
                            </Text>

                            {
                              magazine.previews.length > 0 &&
                                <View style={{ width:'100%', height:(Constant.layout.screenWidth-40) / 3 }}>
                                  <ScrollView
                                  ...
                                  >
                                  {
                                    magazine.previews.map((item, index) => {
                                      return(
                                        <Image
                                          key={ index }
                                          style={{ width:(Constant.layout.screenWidth-40) / 3, height:'100%', marginRight:10, backgroundColor:Constant.color.placeholder, resizeMode:'stretch' }}
                                          source={{ uri:item }} />
                                      )
                                    })
                                  }
                            ...
                            <View style={{ width:'100%', marginVertical:30, marginHorizontal:(Constant.layout.screenWidth - 20 - 244) / 3 }}>
                                <View style={{ flex:1, flexDirection:'row' }}>
                                    <TouchableOpacity
                                      style={{ width:122, height:27, marginRight:(Constant.layout.screenWidth - 20 - 244) / 3 }}
                                      activeOpacity={ 0.8 }
                                      onPress={ () => {this.props.navigation.navigate('MagazineViewer')} }
                                    >
                                        <Image style={{ width:'100%', height:'100%', resizeMode:'stretch' }} source={ imgButton }>
                                            <View style={ [styles.wrapper, {paddingTop: Platform.OS == 'ios' ? 3: 0}] }>
                                                <Text style={{ color:'white', fontFamily:Constant.font.book, fontSize:10, backgroundColor:'transparent' }}>Read Magazine</Text>
                                            </View>
                                        </Image>
                                    </TouchableOpacity>
                                    <TouchableOpacity
                                    ...
                                    >
                                        <Image style={{ width:'100%', height:'100%', resizeMode:'stretch' }} source={ imgButton }>
                                            <View style={[ styles.wrapper, {flexDirection:'row', paddingTop:Platform.OS == 'ios' ? 3: 0} ]}>
                                                <Image style={{ width:19, height:14, marginRight:5 }} source={ iconComment }/>
                                                <Text style={{ color:'white', fontFamily:Constant.font.book, fontSize:10, backgroundColor:'transparent' }}>Rate/Comment</Text>
                                            </View>
                                        </Image>
                                    </TouchableOpacity>
                                </View>
                  ...
                  animationType='fade'>
                    <View style={ styles.wrapper }>
                      <Rating
                        ...
                        onBtnSubmit={ (rate) => {this.onBtnSubmit(rate)} }
